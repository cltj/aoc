...
def solve_part_one(lines: list) -> int:
    commands = (pd.Series(lines).str.split(expand=True)
                .rename(columns={0: 'dir', 1: 'dist'})
                .astype({'dir': str, 'dist': int}))
    commands.loc[commands.dir == 'up', 'dist'] *= -1
    commands.loc[commands.dir == 'up', 'dir'] = 'down'
    commands = commands.groupby(['dir']).sum()
    return int(commands.product(axis=0))
